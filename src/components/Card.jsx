...
const CardPage = ({ pet }) => {
  ...
  return (
    <div
      className="w-full max-w-sm mx-auto bg-white rounded-2xl s
        ...
        <div className="flex gap-3 pt-3">
          ...
          {pet?.status === "Available" && (
            <Link
              ...
              className="flex-1 text-white border border-gray-300 py-2 rounded-xl hover:bg-black transition text-center"
            >
              View Details
            </Link>
          )}
          ...
          {pet?.status === "Available" && (
            <Link
              href={session?.user?.email ? `/all-pets/${pet._id}` : "/login"}
              className="flex-1 bg-pink-500 text-white py-2 rounded-xl hover:bg-pink-600 transition text-center"
            >
              Adopt Now
            </Link>
          )}
